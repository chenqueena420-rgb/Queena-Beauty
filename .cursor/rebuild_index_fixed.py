...
OUT_PATH = ROOT / ".cursor" / "index.html"
...
function watsonsUrl(name) {{
  return "https://www.watsons.com.tw/search?text=" + encodeURIComponent(String(name || "").trim());
}}

function poyaUrl(name) {{
  return "https://www.poyabuy.com.tw/v2/Search?q=" + encodeURIComponent(String(name || "").trim());
}}

function cosmedUrl(name) {{
  return "https://shop.cosmed.com.tw/v2/Search?q=" + encodeURIComponent(String(name || "").trim());
}}
...
function renderCard(row) {{
  ...
  const wUrl = watsonsUrl(p.name);
  const pUrl = poyaUrl(p.name);
  const cUrl = cosmedUrl(p.name);
  return (
    '<article class="card">' +
    ...
    hint +
    '<div class="buy-group">' +
    '<a class="btn-buy wat" href="' +
    escAttr(wUrl) +
    '" target="_blank" rel="noopener noreferrer">屈臣氏</a>' +
    '<a class="btn-buy poya" href="' +
    escAttr(pUrl) +
    '" target="_blank" rel="noopener noreferrer">寶雅</a>' +
    '<a class="btn-buy cosmed" href="' +
    escAttr(cUrl) +
    '" target="_blank" rel="noopener noreferrer">康是美</a>' +
    "</div>" +
