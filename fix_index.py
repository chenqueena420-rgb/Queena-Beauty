...
<footer> ©WAVE WAVE 智慧美妝保養選品</footer>
<script>
const productData = '''
...
function renderCard(row) {
  const p = row.p;
  const em = CAT_EMOJI[p.category] || CAT_EMOJI["精華"];
  ...
  if (p.links) {
      ...
      if (p.links['蝦皮'] || p.links['蝦皮商城']) {
        let sLink = p.links['蝦皮'] || p.links['蝦皮商城'];
        buyButtons += '<a class="btn-buy shopee" href="' + escAttr(sLink) + '" target="_blank" rel="noopener noreferrer" style="background-color: #ee4d2d !important; color: #ffffff !important;">去蝦皮買</a>';
    }
  }
...
function renderChips() {
  const cats = ["全部", ...new Set(all.map(p => p.category))].filter(c => c && c.trim() !== '');
